...
export const getPlanetsFetch = episode_id => (dispatch, getState) => {
  ...
  const ids = planets.map(url => url.split(GET_COLLECTION_URL('planets'))[1].split('/')[0])

  for (let i = 1; i < getMaxOfArray(ids); i++) {
    const options = {
      method: 'GET',
    ...
    axios(options)
      .then(({data: {results}}) => dispatch(get.planets.success(results)))
...
export const getStarshipsFetch = episode_id => (dispatch, getState) => {
  ...
  const ids = starships.map(url => url.split(GET_COLLECTION_URL('starships'))[1].split('/')[0])
  ...
  for (let i = 1; i < getMaxOfArray(ids); i++) {
    ...
    axios(options)
        .then(({data: {results}}) => dispatch(get.starships.success(results)))
